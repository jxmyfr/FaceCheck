...
│  • bullet 3          │                  │   │
│                      └──────────────────┘   │
│ ─────────────────────────── 07 / 21 ──────  │  ← footer: gradient line + page
└──────────────────────────────────────────────┘
```
...
### SLIDE 4 — ทฤษฎี: การจดจำใบหน้าด้วย Deep Learning
**หัวข้อ:** Face Recognition & ArcFace

**Face Recognition ≠ Face Detection:**
- Detection: หาตำแหน่งใบหน้าในภาพ (SCRFD detector)
- Recognition: ระบุตัวตนว่าเป็นใคร

**Metric Learning (ไม่ใช่ Classification):**
- ไม่เรียนรู้ "นักเรียน N คน" แต่เรียนรู้ feature ที่ทำให้แต่ละคนแตกต่างกัน
- ใช้ได้แม้กับใบหน้าที่ไม่เคยเห็นใน training set

**ArcFace Loss (Additive Angular Margin Loss):**
- เพิ่ม angular margin ระหว่าง class → embedding ของคนเดียวกันใกล้กัน, คนต่างกันห่างกัน
- ผลลัพธ์: vector 512 มิติที่ normalized (L2-norm = 1)

**โมเดล:** buffalo_l — ResNet-100 backbone + ArcFace, trained บน MS1MV3 (5.8M รูป, 85K คน)

**ภาพแนะนำ:** diagram แสดง CNN layers → 512-D vector, วงกลม ArcFace margin ระหว่าง 2 class

---

### SLIDE 5 — ทฤษฎี: Embedding Space & Euclidean Distance
**หัวข้อ:** Embedding Space & Distance Metric

**Face Embedding:**
- ใบหน้า 1 รูป → vector 512 มิติ (ตัวเลข 512 ค่า)
- แต่ละมิติ = feature นามธรรมที่ neural network เรียนรู้มา
- normalized: ||v|| = 1 (อยู่บน unit hypersphere)
...
| คู่ | ระยะทาง d |
|-----|-----------|
| คนเดียวกัน (same person) | ~0.5 – 0.8 |
| คนต่างกัน (different person) | ~1.0 – 1.4 |

**Threshold:** ตั้งได้โดย Admin (default 1.0)
- d ≤ threshold → "พบนักเรียน"
- d > threshold → "ไม่พบในระบบ"

**Confidence Score:**
```
confidence = 1 − (d² / 2)
```
ตัวอย่าง: d = 0.6 → confidence = 1 − 0.18 = **82%**

**ภาพแนะนำ:** 2D projection (t-SNE) แสดง cluster ใบหน้าคนเดียวกัน + distance line ระหว่าง 2 จุด
...
### SLIDE 9 — Multi-Angle Enrollment
...
### SLIDE 10 — Quality & Liveness Check
...
### SLIDE 11 — QR Anti-Buddy-Punching
...
### SLIDE 17 — Database Schema (Simplified)
