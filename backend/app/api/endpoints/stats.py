...
def get_stats_by_grade(
    ...
    att_rows = (
        db.query(Student.grade_level, func.count(distinct(AttendanceLog.student_id)).label("today_count"))
...
def get_stats_by_room(
    ...
    att_rows = (
        db.query(Student.room_number, func.count(distinct(AttendanceLog.student_id)).label("today_count"))
        .join(AttendanceLog, AttendanceLog.student_id == Student.id)
        .filter(Student.grade_level == grade_level)
        .filter(func.date(AttendanceLog.timestamp) == today)
        .filter(AttendanceLog.status.in_(["present", "late"]))
