...
*   **Anti-Spoofing (Liveness Check)**: ตรวจสอบว่าเป็นใบหน้าจริง ไม่ใช่รูปถ่ายหรือหน้าจอ ด้วย LBP texture analysis และ FFT frequency analysis
*   **Anti-Buddy-Punching (QR)**: แต่ละ QR token มี `jti` (JWT ID) ที่ไม่ซ้ำกัน เมื่อใช้แล้วบันทึกลง `qr_sessions_used` → ใช้ซ้ำไม่ได้ (1 QR = 1 ครั้ง, หมดอายุ 30 นาที)
*   **Room Lock**: scan endpoint ตรวจสอบว่านักเรียนอยู่ในห้องที่ตรงกับตารางสอน ปฏิเสธถ้าไม่ตรง
*   **Audit Trail**: ทุกการแก้ไข status บันทึกลง `attendance_audit_logs` พร้อมผู้แก้ไข + เวลา + ค่าก่อน/หลัง
*   **On-Premise Data**: ข้อมูลชีวมิติ (embedding vector + รูปใบหน้า) เก็บในระบบโรงเรียน ไม่ส่งออกไปยังบริการภายนอก
