...
graph TB
    subgraph Client["Frontend (React + Vite)"]
        UI[หน้าเว็บ]
        CAM[กล้องเว็บแคม<br/>react-webcam]
    end

    subgraph Server["Backend (FastAPI + Python)"]
        API[REST API<br/>:8000/api/v1]
        FP[FaceProcessor<br/>InsightFace buffalo_l]
        AUTH[JWT Auth<br/>HS256]
        DB[(SQLite / PostgreSQL)]
    end

    subgraph Store["Storage"]
        DBFILE[database.db<br/>embedding vectors<br/>attendance logs]
        FACES[/storage/faces/<br/>face images]
    end
